...
const h1 = $("<h1>").text(Data.title);
video_prt.append(h1);
const para = $("<p>").text(Data.description);
video_prt.append(para);
const hr = $("<hr>");
video_prt.append(hr);

const comment_prt = $(".comment-part");
for (let i = 0; i < 7; i++) {
  const comments = $("<div>").attr("id", "comments");
  const comments_img = $("<div>").addClass("comments-img");
  const image = $("<img>").attr("src", Data.comments[i].image);
  comments_img.append(image);
  comments.append(comments_img);
  const comments_info = $("<div>").addClass("comments-info");
  const h4 = $("<h4>").text(Data.comments[i].name);
  comments_info.append(h4);
  const h5 = $("<h5>").text(Data.comments[i].comment);
  comments_info.append(h5);
  comments.append(comments_info);
  comment_prt.append(comments);
}

const poster_prt = $(".poster-part");
const h3 = $("<h3>").text("Upcoming Projects");
poster_prt.append(h3);
const poster_img = $("<div>").attr("id", "poster-img");
for (let j = 0; j < 3; j++) {
  const img_post = $("<img>")
    .attr("src", posterArray[j].imageUrl)
    .attr("alt", posterArray[j].title);
